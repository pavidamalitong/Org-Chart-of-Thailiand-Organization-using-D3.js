...
function openPaths(paths, oldPaths){
    for(var i =0;i<oldPaths.length;i++){
        ...
        if(oldPaths[i].children && i!=0){ //if children are open: hide them, otherwise: don't do anything
            oldPaths[i]._children = oldPaths[i].children;
            oldPaths[i].children = null;
            }
    }
    ...
    for(var i =0;i<paths.length;i++){
        if(paths[i].id !== "00"){//i.e. not root
            paths[i].class = 'found';
            if(paths[i]._children && i<paths.length){ //if children are hidden: open them, otherwise: don't do anything
