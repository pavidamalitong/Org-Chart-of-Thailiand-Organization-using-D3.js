...
function getData(initData, searchID){
    newData = [];
    if(searchID.length == 2){
        newData = initData.filter(dta => dta.child.substring(0, 2) === searchID);
    }
    else if(searchID.length == 5){
        indexP1 = initData.findIndex(dta => dta.child === searchID.substring(0, 2));

        newData = initData.filter(dta => dta.child.substring(0, 5) === searchID);
        newData.unshift(initData[indexP1]);
    }
    else if(searchID.length == 8){
        indexP1 = initData.findIndex(dta => dta.child === searchID.substring(0, 2));
        indexP2 = initData.findIndex(dta => dta.child === searchID.substring(0, 5));
        indexP3 = initData.findIndex(dta => dta.child === searchID);

        newData.push(initData[indexP1]);
        newData.push(initData[indexP2]);
        newData.push(initData[indexP3]);
    }

    newData[0].parent = undefined;
...
d3.csv("rawData.csv").then(function(fd){
    ...
    $("#search").on("select2-selecting", function(e) {
        newData = getData(initData, e.object.id);
        var paths = searchTree(newData,e.object.id,[]);
        console.log(newData);
